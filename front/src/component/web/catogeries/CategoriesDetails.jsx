...
export default function CategoriesDetails() {
    ...
    const { UserToken } = useContext(UserContext);
      ...
      const getPagination =  async(Page=1,limit=6)=>{
        setLoading(true);
         try{
         const {data} = await axios.get(`https://ai-o49a.onrender.com/products/${catogoryId}?page=${page}&limit=${limit}`);
          console.log(data);
          setproduct(data);
          console.log(loading);
          return data;
    ...
    return (
     ...
     {!loading?<div className='row row-gap-5' >
  ...
  <path d="M6.5 13a6.47 6.47 0 0 0 3.845-1.258h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1A6.47 6.47 0 0 0 13 6.5 6.5 6.5 0 0 0 6.5 0a6.5 6.5 0 1 0 0 13m0-8.518c1.664-1.673 5.825 1.254 0 5.018-5.825-3.764-1.664-6.69 0-5.018"/>
     ...
     <img height='500px' className='position-absolute h-100 z-0 w-100 rounded-3'    src={product.mainImage.secure_url}/>
      ...
      </div>:<div className='loading w-100 h-100 z-3'><span className="loader "></span></div>}
